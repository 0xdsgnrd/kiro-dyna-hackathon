...
const DevLogVideo: React.FC = () => {
  ...
  const sessionDuration = fps * 4; // 4 seconds per session
  
  // Development sessions from DEVLOG
  const sessions = [
    {
  ...
  // Animation progress within current session
  ...
  return (
    <div style={{
      ...
      {frame < titleDuration && (
        <div style={{
          textAlign: 'center',
          opacity: Math.min(1, frame / 30), // Fade in over 1 second
          transform: `translateY(${Math.max(0, 50 - frame * 2)}px)` // Slide up
        }}>
          <h1 style={{
            fontSize: '72px',
            fontWeight: 'bold',
          ...
          <div style={{
            ...
            Total Development Time: 17.8 hours
          </div>
        </div>
      )}
      ...
      {frame >= titleDuration && currentSession && (
        <div style={{
          width: '100%',
          maxWidth: '800px',
          opacity: Math.min(1, sessionProgress * 4), // Fade in quickly
          transform: `translateX(${Math.max(0, 100 - sessionProgress * 200)}px)` // Slide in from right
        }}>
